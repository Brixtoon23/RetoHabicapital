...
def transfer(data: TransferRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    destination = db.query(Account).filter(Account.id == data.to_account_id).with_for_update().first()
    ...
    db.commit()
    return {"message": "Transferencia exitosa", "amount_cents": data.amount_cents}